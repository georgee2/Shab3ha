...
def get_items(pos_profile, price_list=None, item_group="", search_value=""):
    ...
    def _get_items(pos_profile, price_list, item_group, search_value):
        ...
        # Define the filters and fields you want to retrieve
        filters = {
            "disabled": 0,
            "is_sales_item": 1,
            "is_fixed_asset": 0,
        ...
        # Include the child table in the fields
        # fields.append({
        ...
        # Fetch data using frappe.get_list
        items_data = frappe.get_list(
            doctype="Item",
            filters=filters,
            fields=fields,
        ...
        #get all notes in item from childtable custom_notes from doctype Notes
        for so in items_data:
            note = frappe.get_all(
                'Notes',
                filters={"parent": so.item_code},
                fields=['note']
            )
            so["notes"] = [i.get("note") for i in note]

        for go in items_data:
            note = frappe.get_all(
                'All Notes',
                filters={"parent": so.item_code},
                fields=['note']
            )
            go["selected_notes"] = [i.get("note") for i in note]
...
def create_customer(
    ...
    if method == "create":
        ...
        # customer_doc.custom_first_mobile = first_mobile
        if second_mobile:
            customer_doc.custom_second_mobile = second_mobile
        if address:
            customer_doc.custom_city = address
        # if area:
        #     customer_doc.custom_area = area
        # if building:
        #     customer_doc.custom_department = building
        # if floor:
        #     customer_doc.custom_floor = floor
