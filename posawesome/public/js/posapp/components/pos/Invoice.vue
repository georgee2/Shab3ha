      ...
      <v-row no-gutters>
        <v-col cols="7">
          <v-row no-gutters class="pa-1 pt-1 pr-1 mr-3">
            ...
            <v-col cols="6" class="pa-1">
              <v-btn-toggle v-model="customer_type" rounded dense color="primary">
...
export default {
  ...
  methods: {
    // initializeSelectedNotes() {
    //   this.items.forEach(item => {
    //     if(!this.selectedNotes.hasOwnProperty(item.item_name)){
    //       this.selectedNotes[item.item_name] = item.notes.reduce((acc, note) => {
    //         acc[note] = false;
    //         return acc;
    //       }, {});;
    //     }
    //   });
    // },
    // isNoteSelected(item, note) {
    //   // Replace this with the actual logic to determine if the note is selected
    //   return this.items[item].selected_notes.includes(note);
    // },
    // handleCheckboxChange(index, note) {
    //   if(!this.items[index]["selected_notes"].includes(note)) {
    //     this.items[index]["selected_notes"].push(note);
    //   }else {
    //     const indexToRemove = this.items[index]["selected_notes"].indexOf(note);
    //     this.items[index]["selected_notes"].splice(indexToRemove, 1);
    //   }
    //   console.log("v-model select:" + (this.items[item].selected_notes.includes(note)));
    //   console.log("after select:" + this.items[index]["selected_notes"]);
    // },
    remove_item(item) {
    ...
    get_invoice_doc() {
      ...
      // doc.customer_type = cust.customer_type;
      return doc;
    },
    get_invoice_items() {
      const items_list = [];
    ...
    quick_pay_payment() {
      ...
      const vm = this;
      // frappe.call({
      //   method: "posawesome.posawesome.api.posapp.submit_quick_entry",
      //   args: {
      //     screens_no: this.kitchen_items,
      //   },
      //   async: true,
      //   callback: function (r) {
      //     if (r.message) {
